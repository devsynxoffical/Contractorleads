...
function Footer() {
  return (
    <footer className="relative border-t border-border bg-[var(--canvas-deep)]">
      <div className="mx-auto grid max-w-6xl gap-10 px-5 py-16 sm:px-8 md:grid-cols-4">
        <div className="md:col-span-1">
          <div className="flex items-center gap-2">
            ...
            <span className="font-[family-name:var(--font-display)] text-[15px] font-semibold text-ink">
              Contractor Leads
            </span>
          </div>
          <p className="mt-3 text-[13px] leading-relaxed text-ink-muted">
            Premium lead intelligence for agencies that sell to home-service contractors.
          </p>
        </div>
        {[
          { h: "Product", links: [["Features", "#features"], ["Pricing", "#pricing"], ["FAQ", "#faq"]] },
          { h: "Company", links: [["Sign in", "/login"], ["Register", "/register"], ["Contact", "mailto:hello@contractorleads.us"]] },
          { h: "Legal", links: [["Privacy", "#"], ["Terms", "#"]] },
        ].map((col) => (
          <div key={col.h}>
            <p className="text-[12px] font-semibold uppercase tracking-wider text-ink-faint">{col.h}</p>
            <ul className="mt-3 space-y-2">
              {col.links.map(([label, href]) => (
                <li key={label}>
                  <a href={href} className="text-[13px] text-ink-muted hover:text-ink">
                    {label}
                  </a>
                </li>
              ))}
      ...
      <div className="border-t border-border px-5 py-5 text-center text-[12px] text-ink-faint sm:px-8">
