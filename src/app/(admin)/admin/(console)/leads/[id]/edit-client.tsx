...
export default function AdminLeadEditClient({
  from,
}: {
  from?: string;
}) {
  const { id } = useParams<{ id: string }>();
  const fromScrape = from === "scrape";
  ...
  if (!lead) {
    return (
      <p className="animate-pulse text-sm text-ink-muted">Loading lead…</p>
    );
  }
      ...
      <AdminPageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Link
              href={`/admin/leads/${id}${fromScrape ? "?from=scrape" : ""}`}
          ...
          <section className="rounded-2xl border border-border/80 bg-white p-5 shadow-[var(--shadow-card)]">
            ...
            <ul className="mt-2 space-y-1 text-[13px] text-ink-muted">
              {lead.savedBy.map((s, i) => (
                <li key={i}>{s.user.companyName || s.user.email}</li>
              ))}
            </ul>
          </section>
        )}
