...
const promptFieldForType: Record<string, "emailPrompt" | "smsPrompt" | "followupPrompt" | "salesScriptPrompt"> = {
  ...
  sales_script: "salesScriptPrompt",
};
...
export async function POST(request: Request) {
  ...
  if (!leadId || !type || !DEFAULT_OUTREACH_PROMPTS[type]) {
...
`;
  ...
  if (!apiKey) {
    ...
    const typePrompt = resolvePrompt(
      brain,
      promptFieldForType[type],
      DEFAULT_OUTREACH_PROMPTS[type],
    );
    const { text } = await generateText({
      model: openai(brain?.outreachModel ?? "gpt-4o-mini"),
